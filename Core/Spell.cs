...
namespace DefaultCombat.Core
{
    ...
    public static class Spell
    {
        ...
        {
            return new Decorator(ret => BuddyCron.Core.Player.IsCasting, new Action(ret => RunStatus.Success));
        }
        ...
        {
            return Cast(spell, ret => Me.Target, reqs);
        }
        ...
        public static Composite CastOnGround(string spell, Selection<bool> reqs = null)
        {
            return
                new Decorator(
                    ret =>
                        (reqs == null || reqs(ret)) && Me.Target != null,
                    CastOnGround(spell, ctx => Me.Target.Location, ctx => true));
        }
        ...
        {
            return DoTGround(spell, ret => BuddyCron.Core.Player.Target, time, reqs);
        }
        ...
        {
            return DoT(spell, ret => Me.Target, debuff, time, reqs);
        }
